...
def prepare_hh_ss(model):
    ...
    if par.Nfix > 1:
        for i_fix in range(par.Nfix):
            par.z_grid[:],ss.z_trans[i_fix,:,:],e_ergodic,_,_ = log_rouwenhorst(par.rho_z,par.sigma_psi,n=par.Nz)
    else:
...
def find_ss(model,do_print=False):
    ...
    if do_print:
        ...
        print(f'Implied Z/Y = {ss.Z/ss.Y:6.3f}') 
        print(f'Implied G = {ss.G:6.3f}')
